...
/***************************************************************************
 ...
 Modified from the original module of propertypages.py of Qgis2threejs plugin
...
class ControlsPropertyPage(PropertyPage, Ui_ControlsPropertiesWidget):
    ...
    def setup(self, properties=None):
        ...
        if properties:
            ...
            controls = QSettings().value("/PhyloGeoRec/lastControls", def_vals.controls, type=unicode)
...
#    QToolTip.showText(self.horizontalSlider_DEMSize.mapToGlobal(QPoint(0, 0)), "about {0} x {0}".format(size), self.horizontalSlider_DEMSize)
    ...
    def selectLayerClicked(self):
        if QGis.QGIS_VERSION_INT < 20400:
            QMessageBox.warning(self, "PhyloGeoRec", "Sorry, newer QGIS version (>= 2.4) is required.")
            return
